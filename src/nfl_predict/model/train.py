...
- `data/backtest.json` - the weekly walk-forward replay over the holdout seasons: per-season
  and overall metrics for the model and for Vegas, plus calibration bins. Committed, because it
  is the model's stated track record before going live and the site shows it; regenerated by
  every retrain so it always describes the current model.

The model artifact is gitignored. The committed sources of truth are `data/predictions`,
`data/odds`, `data/results`, and this backtest.
"""
